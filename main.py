    ...
    financial_records = [
        {
            ...
            "الكابتن": "أحمد محمود",
            "القيمة الأساسية (ج.م)": 1500.0,
            "الحالة": "مكتملة",
        },
        {
            "الرحلة": "TR-102",
            "العميل": "مصنع النور للبلاستيك",
            "الكابتن": "محمد علي",
            "القيمة الأساسية (ج.م)": 3200.0,
            "الحالة": "مكتملة",
        },
    ]
...
if st.session_state["current_page"] == "العملاء":
    st.markdown(
        "<h2 style='color: #1E88E5;'>🛒 بوابة طلب الشحنات ونظام المزايدة الحي</h2>",
    ...
    suggested_price = st.number_input(
        "السعر المقترح من قبلك للرحلة (جنيه):",
        min_value=0.0,
        step=50.0,
        value=150.0,
    )
    ...
    if suggested_price > 0 and customer_name and delivery_address:
        st.write("---")
        st.markdown(
            "<h4 style='color: #1E88E5;'>🚖 العروض المستلمة حية من الكباتن المحيطين بك:</h4>",
            unsafe_allow_html=True,
        )
        ...
        with c1:
            st.markdown(
                f"<div style='border: 1px solid #ddd; padding: 10px; border-radius: 8px; text-align: center; margin-bottom: 10px;'><b>كابتن/ رأفت المنسي</b><br>⭐ 4.9 (موتوسيكل)<br><h3 style='color: #1E88E5; margin: 5px 0;'>{bid_1:,.0f} ج.م</h3></div>",
                unsafe_allow_html=True,
            )
            if st.button(
                "قبول عرض رأفت", key="accept_bid_1", use_container_width=True
            ):
                new_trip = {
                    "الرحلة": f"TR-{len(financial_records)+101}",
                    "العميل": customer_name,
                    "الكابتن": "رأفت المنسي",
                    "القيمة الأساسية (ج.م)": float(bid_1),
                    "الحالة": "مكتملة",
                }
        ...
        with c2:
            st.markdown(
                f"<div style='border: 1px solid #ddd; padding: 10px; border-radius: 8px; text-align: center; margin-bottom: 10px;'><b>كابتن/ إيهاب جلال</b><br>⭐ 4.8 (سيارة فاني)<br><h3 style='color: #1E88E5; margin: 5px 0;'>{bid_2:,.0f} ج.م</h3></div>",
                unsafe_allow_html=True,
            )
            if st.button(
                "قبول عرض إيهاب", key="accept_bid_2", use_container_width=True
            ):
                new_trip = {
                    "الرحلة": f"TR-{len(financial_records)+101}",
                    "العميل": customer_name,
                    "الكابتن": "إيهاب جلال",
                    "القيمة الأساسية (ج.م)": float(bid_2),
                    "الحالة": "مكتملة",
                }
                if save_to_firebase("financial_records", new_trip):
                    st.success(
                        f"☁️ تم ترحيل رحلة كابتن إيهاب وتأمين الحسابات سحابياً!"
                    )
                    st.rerun()

        with c3:
            st.markdown(
                f"<div style='border: 1px solid #ddd; padding: 10px; border-radius: 8px; text-align: center; margin-bottom: 10px;'><b>كابتن/ مصطفى طه</b><br>⭐ 4.7 (سيارة ربع نقل)<br><h3 style='color: #1E88E5; margin: 5px 0;'>{bid_3:,.0f} ج.م</h3></div>",
                unsafe_allow_html=True,
            )
            if st.button(
                "قبول عرض مصطفى", key="accept_bid_3", use_container_width=True
            ):
                new_trip = {
                    "الرحلة": f"TR-{len(financial_records)+101}",
                    "العميل": customer_name,
                    "الكابتن": "مصطفى طه",
                    "القيمة الأساسية (ج.م)": float(bid_3),
                    "الحالة": "مكتملة",
                }
                if save_to_firebase("financial_records", new_trip):
        ...
        st.warning(
            "⚠️ يرجى إدخال اسم العميل وعنوان التوصيل لتشغيل لوحة التفاعل الحية."
        )

# --- ب: بوابة الكباتن وتوثيق المستندات الفنية ---
elif st.session_state["current_page"] == "الكباتن":
    st.markdown(
        "<h2 style='color: #4CAF50;'>🚖 مركز توثيق الفحص الأمني للمستندات (الكباتن)</h2>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        "<h4 style='color: #4CAF50;'>🪪 1. صور بطاقة الرقم القومي السارية</h4>",
        unsafe_allow_html=True,
    )
    id_col1, id_col2 = st.columns(2)
    with id_col1:
        id_front = st.file_uploader(
            "البطاقة الشخصية (وش):", type=["jpg", "png", "jpeg"], key="idf"
        )
    with id_col2:
        id_back = st.file_uploader(
            "البطاقة الشخصية (ظهر):", type=["jpg", "png", "jpeg"], key="idb"
        )

    st.markdown(
        "<h4 style='color: #4CAF50;'>🪪 2. صور رخصة القيادة الشخصية السارية</h4>",
        unsafe_allow_html=True,
    )
    license_col1, license_col2 = st.columns(2)
    with license_col1:
        license_front = st.file_uploader(
            "رخصة القيادة (وش):", type=["jpg", "png", "jpeg"], key="lf"
        )
    with license_col2:
        license_back = st.file_uploader(
            "رخصة القيادة (ظهر):", type=["jpg", "png", "jpeg"], key="lb"
        )

    st.markdown(
        "<h4 style='color: #4CAF50;'>🚖 3. صور رخصة المركبة (السيارة/الموتوسيكل)</h4>",
        unsafe_allow_html=True,
    )
    vehicle_col1, vehicle_col2 = st.columns(2)
    with vehicle_col1:
        vehicle_front = st.file_uploader(
            "رخصة المركبة (وش):", type=["jpg", "png", "jpeg"], key="vf"
        )
    with vehicle_col2:
        vehicle_back = st.file_uploader(
            "رخصة المركبة (ظهر):", type=["jpg", "png", "jpeg"], key="vb"
        )

    if st.button(
        "تفعيل واعتماد الكابتن وحفظه سحابياً", use_container_width=True
    ):
        if (
            driver_name
            and driver_id
            ...
            if save_to_firebase("verified_drivers", driver_profile):
                st.success(
                    f"🎉 تم اعتماد الكابتن [{driver_name}] وحفظ ملفه الرقمي في سحابة Firebase بنجاح!"
                )
        else:
            st.error("❌ تعذر التوثيق! يرجى رفع كافة المستندات المطلوبة كاملة.")

# --- ج: النظام الإداري والمالي المحاسبي والأتمتة الشاملة ---
elif st.session_state["current_page"] == "المالي":
    st.markdown(
        "<h2 style='color: #9C27B0;'>📊 الهيكل المحاسبي والتقرير الضريبي وعوائد المزايدات السحابية</h2>",
        unsafe_allow_html=True,
    )
    ...
    df["إجمالي الفاتورة بالضريبة"] = (
        df["القيمة الأساسية (ج.م)"] + df["القيمة المضافة (14%)"]
    )
    ...
    c3.metric(
        "صافي أرباح منصة مُنجز (20%)",
        f"{monjez_net_profit:,.2f} ج.م",
        "⚙️ صافي العمولة",
    )
    ...
    st.write("---")
    st.subheader("⭐ تقييم الجودة وبلاغات الدعم الفني")
    client_test_email = st.text_input(
        "بريد العميل لإرسال التنبيه والفاتورة:", value="client@example.com"
    )
    driver_test_email = st.text_input(
        "بريد المندوب/السائق المعتمد للرحلة:", value="driver@example.com"
    )
    user_rating = st.slider("تقييم العميل النهائي للجودة:", 1, 5, 5)

    if st.button(
        "إغلاق الدورة المستندية الضريبية وشحن التقرير للإدارة والأتمتة",
        use_container_width=True,
    ):
        # 📦 تجهيز حزمة البيانات اللوجستية الموحدة لـ Zoho Flow
        payload = {
            "customer_email": client_test_email,
            "driver_email": driver_test_email,
        ...
        with st.spinner(
            "جاري معالجة المحاسبة وإطلاق الإشارة الحية لـ Zoho Flow..."
        ):
            try:
                # إرسال حزمة البيانات كاملة للـ Webhook
            ...
            if flow_success:
                st.success(
                    "✅ معجزة رقمية! التقطت لوحة Zoho Flow الإشارة الحية بنجاح، وستتولى الأتمتة إشعار العميل والمندوب والدعم فوراً دون تشتيت! 🔐"
                )
            else:
                st.error(
                    "❌ فشل الإرسال، يرجى التحقق من صحة رابط الـ Webhook الفعال في ملف الإعدادات."
                )

# --- د: الصفحة الرئيسية ---
else:
    st.markdown(
        "<h3 style='color: #FF5733;'>🧪 مركز اختبار الإشعارات والربط السحابي</h3>",
        unsafe_allow_html=True,
    )
    st.info(
        "المنصة متصلة الآن بسحابة قاعدة بيانات Firebase بنجاح وبأعلى معايير الحماية والتأمين البرمجي المانع للأخطاء."
    )
