...
def render_parcels_page(user_name="أحمد مصطفى", send_to_firebase=None, send_system_email=None, *args, **kwargs):
    ...
    with st.form("parcels_form", clear_on_submit=True):
        c_name = st.text_input("👤 اسم العميل", value=user_name if isinstance(user_name, str) else "أحمد مصطفى")
...
def render_taxi_page(user_name="أحمد مصطفى", send_to_firebase=None, send_system_email=None, *args, **kwargs):
    ...
    with st.form("taxi_form", clear_on_submit=True):
        c_name = st.text_input("👤 اسم العميل", value=user_name if isinstance(user_name, str) else "أحمد مصطفى")
        o_details = st.text_area("📝 تفاصيل المشوار والوجهة", placeholder="مثال: التوصيل من شارع التحرير إلى الدقي...")
        s_price = st.number_input("💰 الميزانية المقترحة للرحلة (جنيه)", min_value=10, value=50, step=5)
        c_phone = st.text_input("📱 رقم هاتف التواصل", value="+20 1000000000")
...
def render_wallet_page(user_name="أحمد مصطفى", *args, **kwargs):
    ...
    with col_btn:
        with st.popover("➕ إضافة", use_container_width=True):
            topup_amount = st.number_input("المبلغ المراد إضافته", min_value=10, value=100)
            if st.button("دفع", type="primary", use_container_width=True):
                st.info(f"جاري تحضير معاملة شحن بـ {topup_amount} ج.م...")
