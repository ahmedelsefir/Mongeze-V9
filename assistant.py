...
# ✅ الدالة الجديدة: التحقق من API Key
def verify_api_key() -> bool:
    """
    تتحقق من وجود وصحة الـ API Key المحفوظ في st.secrets.
    ترجع True لو المفتاح موجود وصالح، False لو فيه مشكلة.
    """
    try:
        api_key = st.secrets["api"]["key"]
        if not api_key or api_key.strip() == "":
            st.error("❌ API Key غير موجود أو فارغ في إعدادات النظام.")
            return False
        return True
    except KeyError:
        st.error("❌ لم يتم العثور على مفتاح [api][key] في st.secrets — تأكد من إعدادات Streamlit.")
        return False
    except Exception as e:
        st.error(f"❌ خطأ في التحقق من API Key: {e}")
        return False
...
def main():
    ...
    # ✅ التحقق من API Key أول حاجة قبل أي عملية
